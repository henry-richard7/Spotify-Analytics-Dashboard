...
def get(recently_played_df_):
    ...
    weekly_df['day_type'] = weekly_df['played_at'].apply(lambda x: 'weekday' if x.weekday() < 5 else 'weekend')


    years = weekly_df['played_at'].dt.year.unique().tolist()

    week_min = int(weekly_df['week_number'].min())
    week_max = int(weekly_df['week_number'].max())

    year_value = st.selectbox("Select Year",years)
    week_value = st.slider("Select Week",min_value=week_min,max_value=week_max,value=week_max)
    
    recently_played_df = weekly_df[(weekly_df['week_number'] == week_value) & (weekly_df['played_at'].dt.year == year_value)]
    ...
    song_fig = px.bar(
        song_df, x="song", y="duration", text="duration",
    ).update_layout(
    xaxis_title = "Songs",
    ...
    song_duration = f"{song_duration} Minutes" if song_duration < 60 else f"{round(song_duration/60,2)}"
    ...
    artist_df = recently_played_df.groupby("artist").agg({'duration':'sum','artist_image':'first'}).sort_values('duration',ascending=False).reset_index()
    artist_df['duration'] = round(artist_df['duration']/60000,2)
    artist_df =artist_df.sort_values("duration", ascending=False).head(10)

    artist_fig =px.bar(
        artist_df, x="artist", y="duration", text="duration"
    ).update_layout(
    xaxis_title = "Artists",
    yaxis_title = "Duration (Minutes)",
    showlegend=False,
    ...
    album_fig =px.bar(
        album_df, x="album_name", y="duration", text="duration"
    ).update_layout(
    xaxis_title = "Albums",
    yaxis_title = "Duration (Minutes)",
    showlegend=False,
    ...
    album_duration = f"{album_duration} Minutes" if album_duration < 60 else f"{round(album_duration/60,2)} Hours"
    ...
    explicit_fig = go.Figure(data=[go.Pie(labels=explicit_df['explicit'], 
                                      values=explicit_df['duration'], 
                                      pull=[0.2, 0, 0, 0]),
                               ]).update_traces(marker=dict(colors=['red','green']))
    explicit_fig.update_layout(height = 600,
    width = 600)
    
    col2.header("Explicit vs Non Explicit Content")
    col2.write(explicit_fig)

    top_explicit_df = recently_played_df[recently_played_df['explicit'] == "Explicit Content" ]
    ...
    top_non_explicit_df = recently_played_df[recently_played_df['explicit'] == "Non Explicit Content" ]
    top_non_explicit_df = top_non_explicit_df.groupby('song')['duration'].sum().reset_index()
    ...
    top_explicit_song = top_explicit_df['song'].tolist()[0]
    top_explicit_song_duration = top_explicit_df['duration'].tolist()[0]

    top_explicit_song_duration = f"{top_explicit_song_duration} Minutes" if top_explicit_song_duration < 60 else f"{round(top_explicit_song_duration/60,2)} Hours"

    top_non_explicit_song = top_non_explicit_df['song'].tolist()[0]
    top_non_explicit_song_duration = top_non_explicit_df['duration'].tolist()[0]

    top_non_explicit_song_duration = f"{top_non_explicit_song_duration} Minutes" if top_non_explicit_song_duration < 60 else f"{round(top_non_explicit_song_duration/60,2)} Hours"

    col2.write(f"You have listened <b>{unique_explicit_count} Explicit Content Songs</b> out of which you liked <b><u>{top_explicit_song}</u></b> the most as you spent <b>{top_explicit_song_duration}</b>.",unsafe_allow_html=True)
    col2.write(f"You have listened <b>{unique_non_explicit_count} Non Explicit Content Songs</b> out of which you liked <b><u>{top_non_explicit_song}</u></b> the most as you spent <b>{top_non_explicit_song_duration}</b>.",unsafe_allow_html=True)
    
    # Streams By Weekday
    col1.header("Streams Per Week Day")
    weekday_df = recently_played_df.groupby(recently_played_df['played_at'].dt.strftime("%A"))['duration'].sum().reset_index()
    weekday_df['duration'] =  round(weekday_df['duration']/60000,2)
    weekday_df['played_at'] = Categorical(weekday_df["played_at"], categories=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], ordered=True)
    weekday_df = weekday_df.sort_values('played_at')

    weekday_fig = px.bar(
        weekday_df, x="played_at", y="duration", text="duration"
    ).update_layout(
    ...
    max_duration_describtion = f"{max_duration} Minutes" if max_duration < 60 else f"{round(max_duration/60,2)} Hours"

    col1.markdown(f"On **{max_weekday_name}** You Have Spent **{max_duration_describtion}**.")
    # Weekday vs Weekend
    col2.header("Weekends vs Weekdays")
    weekday_end_df = recently_played_df.groupby('day_type')['duration'].sum().reset_index()
    ...
    weekday_end_duration = f"{weekday_end_duration} Minutes" if weekday_end_duration < 60 else f"{round(weekday_end_duration/60,2)} Hours."


    
    col2.write(weekday_end_fig)
    col2.markdown(f"You spend most of your time listening to songs in **{weekday_end_names}s** with a duration of **{weekday_end_duration}**")
    ...
    decades_duration = f"{round(decades_df['duration'].tolist()[0]/60)} Hours" if decades_df['duration'].tolist()[0] > 60 else f"{decades_df['duration'].tolist()[0]} Minutes"

    decades_fig =px.bar(
        decades_df, x="decade", y="duration", text="duration"
    ).update_layout(
    xaxis_title = "Decades",
    yaxis_title = "Duration (Minutes)",
    showlegend=False,
    ...
    col2.markdown(f"You Liked Listening To **{decades_name}** songs as you have spent **{decades_duration}**")

    # Top 5 Genres
    genres_df = recently_played_df[recently_played_df['genres'] != ''].groupby("genres")["duration"].sum().reset_index()
    genres_df['duration'] = round(genres_df['duration']/60000,2)
    genres_df = genres_df.sort_values("duration", ascending=False).head(5)

    genres_fig = go.Figure(data=[go.Pie(labels=genres_df['genres'], 
                                      values=genres_df['duration'], 
                                      pull=[0.2, 0, 0, 0]),
                               ])
    
    genres_fig.update_layout(height = 600,
    width = 600)
    ...
    col1.markdown(f"You mostly like to listen to **{genres}** as you have spent **{str(genres_durations)+' Minutes' if genres_durations < 60 else str(round(genres_durations/60,2))+' Hours'}** !!")

    st.header("Audio Features")

    kpis = ['danceability', 'energy', 'valence', 'loudness', 'acousticness', 'speechiness', 'instrumentalness']
    ...
    suggested_songs = artist_meta.get_recommendations(fav_artist_id,energy,danceability,valence,loudness,acousticness,speechiness,instrumentalness)

    with audio_feature_data.expander("Recommended Songs"):
        st.header("Songs you might like!")
        for data in suggested_songs:
            st.markdown(embed_spotify.embed_track("track",data['trackId']),unsafe_allow_html=True)
    ...
    artist_col2.markdown(f"You have spent: **{milliseconds_duration.get(fav_artist_duration)}**")
