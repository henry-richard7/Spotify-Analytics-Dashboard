...
def get(recently_played_df_):
    ...
    yearly_df = weekly_df[weekly_df["played_at"].dt.year == year_value]
    months = yearly_df["month_type"].unique().tolist()

    month_value = st.selectbox("Select Month", months)

    recently_played_df = yearly_df[weekly_df["month_type"] == month_value]
    ...
    if top_explicit_song_duration:
        col2.write(
            f"You have listned <b>{unique_explicit_count} Explicit Content Songs</b> out of which you liked <b><u>{top_explicit_song}</u></b> the most as you spent <b>{top_explicit_song_duration}</b>.",
            unsafe_allow_html=True,
        )

    if top_non_explicit_song_duration:
        col2.write(
            f"You have listned <b>{unique_non_explicit_count} Non Explicit Content Songs</b> out of which you liked <b><u>{top_non_explicit_song}</u></b> the most as you spent <b>{top_non_explicit_song_duration}</b>.",
            unsafe_allow_html=True,
        )
    ...
    col1.markdown(
        f"You mostly like to listnen to **{genres}** as you have spent **{str(genres_durations)+' Minutes' if genres_durations < 60 else str(round(genres_durations/60,2))+' Hours'}** !!"
    )
