...
def get(recently_played_df_):
    ...
    weekly_df['month_type'] = weekly_df['played_at'].dt.strftime('%B')


    years = weekly_df['played_at'].dt.year.unique().tolist()

    year_value = st.selectbox("Select Year",years)
    months = weekly_df['month_type'].unique().tolist()

    month_value = st.selectbox("Select Month",months)
    
    recently_played_df = weekly_df[(weekly_df['month_type'] == month_value) & (weekly_df['played_at'].dt.year == year_value)]

    total_duration = recently_played_df['duration'].sum()
    total_duration = milliseconds_duration.get(total_duration)

    st.title(f"Spotify Report for {year_value} - {month_value} Month")
    ...
    song_description = f"""
    You have listned <b>{unique_count_songs} songs</b> out of which you liked <b><u>{songs}</u></b> the most as you spent <b>{song_duration}</b>.
    """
    col1.markdown(song_description,unsafe_allow_html=True)
    # End of Top 10 Songs Calculations ------>

    # Top 10 Artists Calculations ----->
    artist_df = recently_played_df.groupby("artist")["duration"].sum().reset_index()
    ...
    artists_description = f"You have listned <b>{unique_count_artists} artists</b> out of which you liked <b><u>{artists[0]}</u></b> the most as you spent <b>{str(round(artists_durations[0])) +' Minutes' if round(artists_durations[0]) < 60 else str(round(artists_durations[0]/60)) +' Hours'}.</b>"
    ...
    album_description = f"You have listned <b>{unique_count_albums} albums</b> out of which you liked <b><u>{albums}</u></b> the most as you spent <b>{album_duration}.</b>"
    ...
    col2.write(f"You have listned <b>{unique_explicit_count} Explicit Content Songs</b> out of which you liked <b><u>{top_explicit_song}</u></b> the most as you spent <b>{top_explicit_song_duration}</b>.",unsafe_allow_html=True)
    col2.write(f"You have listned <b>{unique_non_explicit_count} Non Explicit Content Songs</b> out of which you liked <b><u>{top_non_explicit_song}</u></b> the most as you spent <b>{top_non_explicit_song_duration}</b>.",unsafe_allow_html=True)
    ...
    col1.markdown(f"You mostly like to listnen to **{genres}** as you have spent **{str(genres_durations)+' Minutes' if genres_durations < 60 else str(round(genres_durations/60,2))+' Hours'}** !!")
    ...
    audio_feature_data.markdown("### From Analysing audio features pattern")
    audio_feature_data.markdown(f"You {feature_info[features]}")
    

    st.header("Your Favourite Artist")
